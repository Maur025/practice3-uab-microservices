...
| ------ | ------------------------ | ------------------------------------------------------------- |
| GET    | `/api/ventas/ventas`     | Lista todas las ventas registradas.                           |
| POST   | `/api/ventas/ventas`     | Registra una nueva venta y genera la factura asociada.        |
| GET    | `/api/ventas/ventas/:id` | Retorna el detalle de una venta especifica.                   |
| PUT    | `/api/ventas/ventas/:id` | Anula logicamente la venta cuando se envia `estado: ANULADA`. |
...
- **Inventario (`erp-inventory`):** POST a `http://localhost:7803/api/stock/descontar` _(Código preparado y comentado, listo para activación)._
